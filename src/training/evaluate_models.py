...
class ComprehensiveEvaluator:
    ...
    def setup_mlflow(self):
        """Setup MLflow tracking for model evaluation."""
        try:
            ...
            logger.info("MLflow setup completed for evaluation")
    ...
    def evaluate_sae_model(self, use_best: bool = True, k_values: List[int] = [5, 10, 20]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log to MLflow
            self.log_evaluation_to_mlflow(metrics, 'SAE')
    ...
    def evaluate_rbm_model(self, use_best: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log to MLflow
            self.log_evaluation_to_mlflow(metrics, 'RBM')
    ...
    def log_evaluation_to_mlflow(self, metrics: Dict[str, Any], model_type: str):
        ...
        try:
            with mlflow.start_run(run_name=f"{model_type}_evaluation"):
                ...
                if 'binary_accuracy' in metrics:
                    mlflow.log_metric("binary_accuracy", metrics['binary_accuracy'])
                
                # Log model info
                if 'total_parameters' in metrics:
                    mlflow.log_metric("total_parameters", metrics['total_parameters'])
                ...
                for key, value in metrics.items():
                    if key not in ['hyperparameters', 'architecture', 'model_type'] and isinstance(value, (int, float)):
                        mlflow.log_metric(key, value)
                
                # Log model type
                mlflow.log_param("model_type", model_type)
                
                logger.info(f"Logged {model_type} evaluation metrics to MLflow")
                
        except Exception as e:
            logger.warning(f"Failed to log {model_type} metrics to MLflow: {e}")
